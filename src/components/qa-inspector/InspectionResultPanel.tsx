    ...
    <section
      ...
      <header>
        ...
        <p className="mt-3 text-sm leading-6 text-slate-400">
          Findings generated from the inspection input. Evidence appears here as cited output, not
          as raw input.
        </p>
      </header>
      <button
      ...
      {inspectionError ? <p className="mt-3 text-sm text-red-400">{inspectionError}</p> : null}
      {!result ? (
        <section className="mt-6 rounded-xl border border-slate-800 bg-slate-900 p-4">
          <p className="text-sm font-medium text-slate-100">No inspection result yet</p>
          <p className="mt-2 text-sm leading-6 text-slate-400">
            Run the inspection to generate QA findings from the screenshot, DOM, and accessibility
            check.
          </p>
        </section>
      ) : (
              ...
              {result.issues.map((issue) => (
                <button
                  ...
                  className={`w-full p-4 text-left transition ${
                    selectedIssueId === issue.id ? "bg-slate-800" : "hover:bg-slate-800/60"
                  }`}>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-medium text-slate-100">{issue.affectedElement}</p>
